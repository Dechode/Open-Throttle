...
#
	var sa_modified: float = resultant_slip * peak_slip.x * sign(slip.x)
	var sr_modified: float = resultant_slip * peak_slip.y * sign(slip.y) 
	...
	force_vec.x = pacejka(abs(sa_modified), b_lat, c_lat, total_mu, 0, normal_load) * sign(slip.x)
	force_vec.y = pacejka(abs(sr_modified), b_long, c_long, total_mu, 0, normal_load) * sign(slip.y)
